...
class CategoryImagesII extends CategoryImagesII_Plugin {
	...
	protected function process_upload()
	{
		...
		if ( ! move_uploaded_file( $_FILES[ 'category_images_ii' ][ 'tmp_name' ], $original_file ) ) 
			wp_die( sprintf( __( 'Something went wrong. Could not move the uploaded file from <kbd>%1$s</kbd> to <kbd>%2$s</kbd>.', 'category-images-ii' ), $_FILES[ 'category_images_ii' ][ 'tmp_name' ], $original_file ) );
